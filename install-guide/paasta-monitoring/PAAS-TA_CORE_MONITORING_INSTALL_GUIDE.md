...
3. [PaaS-TA 5.5.X 설치](#106)  
   ...
   　　　●  [deploy-openstack-monitoring.sh](#1025)  
   　3.7. [PaaS-TA 설치](#1030)  
   　3.8. [PaaS-TA 로그인](#1032)   

## Executive Summary

본 문서는 PaaS-TA 5.5.X(이하 PaaS-TA)을 수동으로 설치하기 위한 가이드를 제공하는 데 그 목적이 있다.
...
현재 PaaS-TA 5.5.X에서 검증한 IaaS 환경은 AWS, OpenStack 환경이다.
...
```
$ mkdir -p ${HOME}/workspace/paasta-5.5.X/deployment
$ cd ${HOME}/workspace/paasta-5.5.X/deployment
$ git clone https://github.com/PaaS-TA/common.git
$ cd ${HOME}/workspace/paasta-5.5.X/deployment
$ git clone https://github.com/PaaS-TA/paasta-deployment.git -b v5.5.X
...
$ git clone https://github.com/PaaS-TA/monitoring-deployment.git -b v5.5.X  # v5.5.X monitoring-deployment 구성
```

## <div id='109'/>3.3. Stemcell 업로드
Stemcell은 배포 시 생성되는 PaaS-TA VM Base OS Image이며, PaaS-TA 5.5.X은 Ubuntu xenial stemcell 621.94를 기반으로 한다.  
BOSH 로그인 후 다음 명령어를 수행하여 Stemcell을 올린다.  
BOSH_Director_Name는 BOSH 설치 시 사용한 Director 명이고, IaaS에 배포된 환경에 맞게 입력을 한다(aws, azure, gcp, openstack, vsphere, 그외 입력시 bosh-lite)

- Stemcell 업로드 Script의 설정 수정

> $ vi ~/workspace/paasta-5.5.X/deployment/paasta-deployment/bosh/upload-stemcell.sh
```                     
#!/bin/bash
...
```
$ cd ${HOME}/workspace/paasta-5.5.X/deployment/paasta-deployment/bosh
$ ./upload-stemcell.sh
```
...
> $ vi ~/workspace/paasta-5.5.X/deployment/paasta-deployment/bosh/update-runtime-config.sh
```                     
#!/bin/bash
...
PaaS-TA 설치 파일을 내려받으면 ${HOME}/workspace/paasta-5.5.X/deployment/paasta-deployment/cloud-config 디렉터리 이하에 IaaS별 Cloud Config 예제를 확인할 수 있으며, 예제를 참고하여 cloud-config.yml을 IaaS에 맞게 수정한다.  
...
```
$ bosh -e {director_name} update-cloud-config ${HOME}/workspace/paasta-5.5.X/deployment/paasta-deployment/cloud-config/{iaas}-cloud-config.yml
```
...
<td>AWS 환경에 Monitoring 옵션을 적용하여 PaaS-TA 설치를 위한 Shell Script 파일</td>
</tr>
<tr>
<td>deploy-openstack-monitoring.sh</td>
<td>OpenStack 환경에 Monitoring 옵션을 적용하여 PaaS-TA 설치를 위한 Shell Script 파일</td>
</tr>
<tr>
...
bosh_client_admin_secret: "ert7na4jpewsczt"		# BOSH Client Admin Secret('echo $(bosh int ~/workspace/paasta-5.5.X/deployment/paasta-deployment/bosh/{iaas}/creds.yml —path /admin_password))' 명령어를 통해 확인 가능)
...
```
$ chmod +x ${HOME}/workspace/paasta-5.5.X/deployment/paasta-deployment/paasta/*.sh
```
...
> $ vi ${HOME}/workspace/paasta-5.5.X/deployment/paasta-deployment/paasta/deploy-aws-monitoring.sh  

```
bosh -e {director_name} -d paasta -n deploy paasta-deployment.yml \	# PaaS-TA Manifest File  
	-o operations/{IaaS}.yml \					        # AWS 설정  
	-o operations/use-compiled-releases-online.yml \		# PaaS-TA 설치시 공통 컴파일 릴리즈 파일 정보  
	-o operations/use-haproxy.yml \				        # HAProxy 적용  
	-o operations/use-haproxy-public-network.yml \			# HAProxy Public Network 적용  
 	-o operations/use-compiled-releases-haproxy-online.yml \	# PaaS-TA 설치시 HAProxy 컴파일 릴리즈 파일 정보  
...
```
$ cd ${HOME}/workspace/paasta-5.5.X/deployment/paasta-deployment/paasta
$ ./deploy-{IaaS}.sh
```
